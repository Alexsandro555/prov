  ...
  <v-app class="v-cloak--hidden leader">
    <v-container fluid grid-list-xs text-xs-center>
      <v-layout column wrap>
        <header ref="header"
                :class="{header: true, chickens: section===1, cows: section===2, pigs: section===3, rams:section===4, 'main-layout': section===0}">
              ...
              <v-layout row wrap>
                ...
                <v-flex xs2 hidden-md-and-up justify-center>
                  <v-menu :close-on-content-click="false" offset-x>
                    ...
                    <v-list>
                      <v-list-tile>
                        <v-text-field name="find" label="Введите поисковый запрос" @keyup.enter="search"
                  ...
                  <v-layout row wrap>
                    <v-flex md12 lg8 xl8 class="text-md-center">
                      <v-list class="top-menu text-xs-left text-md-center text-sm-left text-lg-center">
                        <v-list-tile class="top-menu__item"><a class="header-menu__link" href="/delivery">доставка и
                            оплата</a></v-list-tile>
                        <v-list-tile class="top-menu__item"><a class="header-menu__link" href="/contacts">контакты</a>
                        </v-list-tile>
                      </v-list>
                    </v-flex>
          ...
          <!--<v-layout align-center justify-start column fill-height>
            ...
            <div class="wrapper">
              <div class="animal-panel hidden-md-and-down">
                <a href="#"><img :class="{borderSel: section===1}" @click.stop="changeSlide('chickens')"
                                 src="{{asset('images/chicken-img-panel.png')}}"/></a>
                <a href="#"><img :class="{borderSel: section===3}" @click.stop="changeSlide('pigs')"
                                 src="{{asset('images/pig-img-panel.png')}}"/></a>
                <a href="#"><img :class="{borderSel: section===2}" @click.stop="changeSlide('cows')"
                                 src="{{asset('images/cow-img-panel.png')}}"/></a>
                <a href="#"><img :class="{borderSel: section===4}" @click.stop="changeSlide('rams')"
                                 src="{{asset('images/bar-img-panel.png')}}"/></a>
              </div>
              @yield('breadcrumbs')
            </div>
                ...
                <v-layout row wrap>
                  <v-flex xs9>
                    <v-list class="footer-top-menu text-xs-left">
                      <v-list-tile class="footer-top-menu__item"><a class="header-menu__link" href="/about">О
                          компании</a></v-list-tile>
                      <v-list-tile class="footer-top-menu__item"><a class="header-menu__link"
                                                                    href="/news/list">Новости</a></v-list-tile>
                      <v-list-tile class="footer-top-menu__item"><a class="header-menu__link" href="/sale">Акции</a>
                      </v-list-tile>
                      <v-list-tile class="footer-top-menu__item"><a class="header-menu__link" href="/delivery">Доставка
                          и оплата</a></v-list-tile>
                      <v-list-tile class="footer-top-menu__item"><a class="header-menu__link"
                                                                    href="/contacts">Контакты</a></v-list-tile>
                    </v-list>
                  </v-flex>
                  <v-flex xs3>
                            ...
                            @foreach($chunkTypeProduct as $typeProduct)
                              <v-list-tile>
                                @if($typeProduct->product_category)
                                  <a
                                    href="/catalog/{{$typeProduct->product_category->url_key}}/{{$typeProduct->url_key}}">{{$typeProduct->title}}</a>
                                @endif
                              </v-list-tile>
                            @endforeach
                          </v-list>
