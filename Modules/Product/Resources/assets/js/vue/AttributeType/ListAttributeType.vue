        ...
        <v-layout wrap row>
            <v-flex xs12 class="text-xs-center">
                <v-card>
                        ...
                        <v-data-table :headers="headers"
                            ...
                            <template slot="items" slot-scope="props">
                                ...
                                <td class="justify-center layout px-0">
                                    ...
                                    <v-btn icon class="mx-0" @click="$router.push('/attributetype/edit/'+props.item.id)">
                    ...
                    <v-card-actions>
                        <v-btn @click="addAttributeType" color="primary" dark class="text-left mb-2">
                            <v-icon>add</v-icon>
                        </v-btn>
                    </v-card-actions>
                </v-card>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapState('AttributeType', ['items', 'loading']),
        },
        methods: {
            addAttributeType() {
                this.add()
                    .then(response => {
                        this.$router.push({name: 'edit-attributetype', params: { id: response.id.toString()}})
                    })
            },
            goToPage(item) {
                let url = '/attributetype/detail/'
            ...
            ...mapActions('AttributeType',{load: GLOBAL.LOAD, add: GLOBAL.ADD, delete: GLOBAL.DELETE, initialization: GLOBAL.INITIALIZATION})
        }
     }
</script>