              ...
              <template slot="items" slot-scope="props">
                ...
                <td class="justify-center layout px-0">
                  ...
                  <v-btn icon class="mx-0"
                         @click="$router.push({name: 'edit-attributes-group-product', params: {id: props.item.id.toString()}})">
      ...
      next(vm => {
        ...
        vm.loadRelations().then(response => {
          vm.load().then(response => {
            vm.$store.commit('SET_VARIABLE',{module: 'attribute_groups', variable: 'loading', value: false}, {root: true})
          })
        })
      })
    },
    computed: {
      ...mapState('attribute_groups', ['items', 'loading']),
    },
    methods: {
      addAttributeGroup() {
