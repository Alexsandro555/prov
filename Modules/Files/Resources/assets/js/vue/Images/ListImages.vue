        ...
        <v-layout wrap row>
            <v-flex xs12 class="text-xs-center">
                <v-card>
                        ...
                        <v-data-table :headers="headers"
                            ...
                            <template slot="items" slot-scope="props">
                                ...
                                <td class="justify-center layout px-0">
                                    ...
                                    <v-btn icon class="mx-0" @click="$router.push('/$TABLE$/edit/'+props.item.id)">
                    ...
                    <v-card-actions>
                        <v-btn @click="addImages" color="primary" dark class="text-left mb-2">
                            <v-icon>add</v-icon>
                        </v-btn>
                    </v-card-actions>
                </v-card>
...
<script>
    import { ACTIONS, GLOBAL } from "@$TABLE$/constants";
    ...
    export default {
              ...
              next(vm => {
                ...
                vm.loadRelations().then(response => {
                  vm.load().then(response => {
                    vm.$store.commit('SET_VARIABLE',{module: '$TABLE$', variable: 'loading', value: false}, {root: true})
                  })
                })
              })
            },
        methods: {
            addImages() {
                this.add()
                    .then(response => {
                        this.$router.push({name: 'edit-$TABLE$', params: { id: response.id.toString()}})
                    })
            },
            deleteItem (item) {
            ...
            ...mapActions('$TABLE$',{load: GLOBAL.LOAD, add: GLOBAL.ADD, delete: GLOBAL.DELETE, initialization: GLOBAL.INITIALIZATION})
        }
     }
</script>